...
import Muplat from '../assets/Muplat.json'
...
let mixinViews = {
  ...
  created () {
    ...
    if (window.bc === undefined) {
      ...
      window.bc.initWithContractJson(Muplat, 'http://127.0.0.1:8545', 'Muplat')
        .then((error) => {
          ...
          if (error) {
            ...
            window.bc.contract('Muplat').GetUser.call((errorReg, res) => {
              if (errorReg) {
                this.bcConnectionError = true
